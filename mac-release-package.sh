...
PACKAGE=RCB-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=RCB-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} RealCoin.exe RealCoinservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe RealCoin.exe
cp installer/lib/JavaExe.exe RealCoinservice.exe
cp -a ${FILES} nxt
...
/Library/Java/JavaVirtualMachines/jdk-14.0.1.jdk/Contents/Home/bin/jpackage -deploy -outdir . -outfile RCB-client -name RCB-installer -width 34 -height 43 -native dmg -srcfiles ${PACKAGE}.jar -appclass com.izforge.izpack.installer.bootstrap.Installer -v -Bmac.category=Business -Bmac.CFBundleIdentifier=org.nxt.client.installer -Bmac.CFBundleName=RCB-Installer -Bmac.CFBundleVersion=${MACVERSION} -BappVersion=${MACVERSION} -Bicon=installer/AppIcon.icns > installer/javapackager.log 2>&1
